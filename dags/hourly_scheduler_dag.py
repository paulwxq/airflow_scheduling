...
def get_hourly_tables(**context):
    ...
    try:
        ...
        tables_response = requests.get(
            f"{scheduler_api_url}/api/v1/tables?frequency=hourly",
            timeout=30
        )
        ...
        logging.info(f"找到 {len(tables)} 个小时级调度表")
        context['ti'].xcom_push(key='hourly_tables', value=tables)
        return tables
    
    except Exception as e:
        logging.error(f"获取小时级调度表失败: {str(e)}")
        raise
...
def generate_execution_plan(**context):
    ...
    try:
        ti = context['ti']
        tables = ti.xcom_pull(task_ids='get_hourly_tables', key='hourly_tables')
        
        if not tables:
            logging.warning("没有小时级调度表，跳过生成执行计划")
...
with DAG(
    ...
    description='每小时执行的调度DAG',
    schedule_interval='@hourly',
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=['smart_scheduler', 'hourly'],
) as dag:
    ...
    get_tables = PythonOperator(
        task_id='get_hourly_tables',
        python_callable=get_hourly_tables,
    )
